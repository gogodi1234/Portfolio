//Data utilities
const parseNA=string =>(string ==='NA' ? undefined : string);  //===內容和資料型態皆要對
const parseDate=string => d3.timeParse('%Y-%m-%d')(string);    //timeparse:string轉Date

function type(d){
    const date=parseDate(d.release_date);
    return{
        budget:+d.budget,   //+ :string轉為int
...
function setupCanvas(barChartData){
    ...
    const chart_margin = {top:80,right:40,bottom:40,left:80};//留空
    ...
    const this_svg=d3.select('.bar-chart-container').append('svg')
                    ...
                    .append('g')
                    .attr('transform', `translate(${chart_margin.left},${chart_margin.top})`) //back-tick
